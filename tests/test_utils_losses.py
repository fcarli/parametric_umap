...
class TestComputeCorrelationLoss:
    ...
    def test_numerical_stability(self):
        ...
        test_cases = [
            ...
            (torch.tensor([1.0, 1.0001, 1.0002, 1.0003]), 
             torch.tensor([2.0, 2.0001, 2.0002, 2.0003])),
            
            # Different magnitudes
            (torch.tensor([1e-8, 2e-8, 3e-8, 4e-8]), 
             torch.tensor([1e8, 2e8, 3e8, 4e8])),
        ]
...
class TestCorrelationLossIntegration:
    ...
    def test_multiple_batch_scenarios(self):
        ...
        scenarios = [
            ...
            (torch.tensor([1.0, 2.0, 3.0, 4.0]), 
             torch.tensor([1.5, 2.2, 3.3, 3.8])),
            
            # Weakly correlated
            (torch.tensor([1.0, 2.0, 3.0, 4.0]), 
             torch.tensor([2.1, 1.8, 3.2, 3.9])),
        ]
